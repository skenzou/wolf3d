...
static inline void 		inter_hor(t_wolf3d *w, int i, double angle)
{
	...
	a.x = (angle != 180) ? w->cam->position.x + (w->cam->position.y - a.y) / tangent :
	w->cam->position.x + (w->cam->position.y - a.y) / -tangent;
	if (a.x < 0 || a.x >=  w->mini_w)
		a.x = (a.x < 0) ? 0 : w->mini_w - 1;
	//printf("---HOR----Point of intersections : x = %f, y = %f o.x = %f, o.y = %f\n", a.x, a.y, o.x, o.y);
	while (w->map->board[(int)floor(a.y / 32)][(int)a.x / 32] == 0)
	{
...
static inline void 		inter_ver(t_wolf3d *w, int i, double angle)
{
	...
	32 - 1 : ceil(w->cam->position.x / 32) * 32 + 32;
